...
const { Container, Button, Card, CardBlock, CardTitle } = Reactstrap
...
const getWeatherInfoAPI = ({ lat, lon }) => {
  ...
  const fullUrl = baseUrl + latParams + lonParams + unitParams + appIdParams
...
class Weather extends React.Component {
  ...
  genIcon() {
    ...
    return <i className={`wi wi-owm-${id}`} />
  }
  render() {
          ...
          <Card>
            <CardBlock style={cardStyle}>
              ...
              {windSpeed && (
                <CardTitle>
                  {this.renderTemp()}
                  <a href="#" style={linkStyle} onClick={this.toggleDegreeType}>
                    &#176;{this.state.isCelcius ? 'C' : 'F'}
                  </a>, {windSpeed} m/s
                </CardTitle>
              )}
            </CardBlock>
